define([
    ...
    function readjustMenuFormHeights() {
        var menu =  $('.fx-editor-menu-container');
        var fm = $('.fx-editor-modular-form-container');

        var mStyle = menu.attr('style');
        var fStyle = fm.attr('style');

        if (typeof mStyle !== typeof undefined && mStyle !== false) {
            menu.attr("style", "");
        }
        if (typeof fStyle !== typeof undefined && fStyle !== false) {
            fm.attr("style", "");
        }

        var menuHght = menu.height();
        var formHght = fm.height();
    ...
    function setFieldSetsFormatting() {
        ...
        $('#fx-editor-form').find('fieldset').each(function(index, element){
            ...
            var nextForGrp = $('#'+this.id).next().hasClass('form-group');

            if(nextForGrp){
                $('#'+this.id).next().css("padding-top", "15px");
            }

            // first button after a fieldset
            var nextBtn = $('#'+this.id).next('.btn');
        ...
        //On Load of the form, only display nested field sets that contain no required fields (interferes with validation)
    ...
    function initializeFormValidation() {
       ...
       //Nav bar drop down menu control
    ...
    Fx_Editor_Form.prototype.buildFormGroup = function (name, value, rule, fieldSet) {
        var div1Width = (value.type.name == ui_Element_Creator.getTypes().LABEL) ? "11":"4",
            div2Width = "7",
            div3Width = "1";
        ...
        var $inputContainer = $('<div class="col-sm-'+div2Width+'" id="'+$inputDivId+'"></div>');
                    ...
                    if (value.type.hasOwnProperty("name")) {
                        if (value.type.name == ui_Element_Creator.getTypes().CLONEBUTTON) {
                            if (rule.hasOwnProperty("type")) {
                                if (rule.type == bootstrapValidator_Utils.getTypes().IS_REQUIRED || rule.type == bootstrapValidator_Utils.getTypes().IS_REQUIRED_CONDITIONAL)   {
                                    $label.append(" " + "<span class='" + requiredCss + "'></span>");
                                }

                            }
                        }
                        else if (value.type.name == ui_Element_Creator.getTypes().LABEL) {
                            if (rule.hasOwnProperty("type")) {
                                if (rule.type == bootstrapValidator_Utils.getTypes().IS_REQUIRED || rule.type == bootstrapValidator_Utils.getTypes().IS_REQUIRED_CONDITIONAL)   {
                                    $label.append(" " + "<span class='" + requiredCss + "'></span>");
                                }
        ...
        var value,
         ...
         for (var name in props) {
          ...
          //  console.log("---------------------------- FORM:::: cache.modulevalidationrules = "+cache.modulevalidationrules + " ruleExists "+ruleExists +' for' +name);
           ...
           //if(obj["parent-path"]!=null) {
              //  console.log("PARENT PATH ============================ "+obj["parent-path"]);
           // }
    ...
    Fx_Editor_Form.prototype.createParentPathObject = function (obj, path, isMapped) {
       ...
       // console.log("+++++++++++++++++++++++ createParentPathObject "+obj.type.name + " path "+ " is mapped");
    ...
    Fx_Editor_Form.prototype.getValues = function () {
      ...
      // console.log("=========== FORM: getValues: cache.modulepathmapping id = "+id);
       ...
       // result[id] = serializeForm;
        ...
        //console.log("validatedModuleForm.parent = "+validatedModuleForm.parent);
       // console.log("validatedModuleForm.entityPath = "+validatedModuleForm.entityPath);
       // console.log("modulePath = "+modulePath);

        if(validatedModuleForm.entityPath !=null)
            result.entityPath = validatedModuleForm.entityPath;

        if(modulePath != undefined && modulePath!= "root" && modulePath!="")  {
            var keysP = [];
            var keysMP = [];
            if(/\./.test(validatedModuleForm.parent)){
                keysP = validatedModuleForm.parent.split('.');
            } else {
                keysP.push(validatedModuleForm.parent);
            }
            if(/\./.test(modulePath)){
                keysMP = modulePath.split('.');
            } else {
                keysMP.push(modulePath);
            }

            //check that the parent (keysP[0]) exists in the module path, otherwise modulePath value is reset 
            if($.inArray(keysP[0], keysMP) >= 0) {
